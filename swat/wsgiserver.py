# JN 9/21/2011: Copied from CherryPy 3.1.2: cherrypy\\wsgiserver\\__init__.py.
# pylint: disable-all
...
class ChunkedReader(object):
    ...
    def __init__(self, f):
        ...
        self._remaining_chunk_len = 0
    
    def read(self, i):
        data = ''
        while len(data) < i and self._has_chunk():
            # Read required data from the current chunk.
            data += self._read_chunk(i - len(data))

        return data

    def _has_chunk(self):
        """ Reads the size of the next chunk if necessary.
            Returns False if no chunk is available.
            Raises an IOError if the socket is closed unexpectedly.
        """
        # If we haven't ended the stream but have read the entire chunk, read
        # the size of the next chunk.
        if not self._has_ended and not self._remaining_chunk_len:
            # Read the trailing crlf for the previous chunk
            if self._need_crlf:
                crlf = self._f.read(2)
                assert crlf == "\r\n"
                self._need_crlf = False

            # Unset/reset the crlf flag to avoid trying to read the crlf twice on timeout.
            raw_line = self._f.readline()
            if not raw_line:
                raise IOError, 'Unexpected EOF when reading chunked data.'

            line = raw_line.strip().split(";", 1)
            self._need_crlf = True

            self._remaining_chunk_len = int(line.pop(0), 16)
            if self._remaining_chunk_len <= 0:
                self._has_ended = True
        
        return not self._has_ended

    def _read_chunk(self, i):
        """ Reads up to i bytes from the current chunk.
        """
        if not self._has_chunk():
            return ''

        # If we are in the middle of a chunk, try to read the amount
        # that was requested by the caller, but don't read past the
        # end of the chunk.
        read_amt = min(self._remaining_chunk_len, i)
        assert read_amt > 0
        data = self._f.read(read_amt)
        self._remaining_chunk_len -= read_amt
        return data
...
class HTTPRequest(object):
    ...
    def _respond(self):
        if self.chunked_read:
            self.environ["wsgi.input"] = ChunkedReader(self.rfile)
...
class CherryPyWSGIServer(object):
    ...
    def bind(self, family, type, proto=0):
        ...
        # Disable reusing addresses in Windows. Per MSDN, the behavior is
        # completely undefined on Windows. Just disable it, which means
        # that all future servers trying to bind to this port will blow up.
        reuse_addr = 0 if os.name == 'nt' else 1
        self.socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, reuse_addr)
