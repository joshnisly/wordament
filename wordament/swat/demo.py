...
# Demonstrates status framework
@swat.status_thread(exclusive=True, exclusive_parms=['component', 'component2'])
def long_process_dynamic(status_key, request, component, component2):
    should_fail = request.JSON.get('should_fail', False)
    swat.thread_append_status_text(status_key, 'Counting to 10: \n')
    ...
    for i in range(0, 10):
        swat.thread_append_status_text(status_key, '%s%s%i\n' % (component, component2, i))
        time.sleep(1)
...
urls = (
    ...
    ('/start/', long_process),
    ('/start/<s>/<s>', long_process_dynamic),
    ('/get_file/[s]/', bogus)
)
