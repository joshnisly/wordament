...
def thread_spawn(func, args=None, kwargs=None):
    ...
    process_thread = _ProcessThread(status_key, func, None, args,
                                    kwargs, exclusive=False)
    process_thread.start()
    return status_key

def thread_spawn_exclusive(func, exclusive_parms=None, args=None, kwargs=None):
    func_name = _calc_func_signature(func, exclusive_parms, args)
    ...
    status_key = _get_new_status_key()
    process_thread = _ProcessThread(status_key, func, exclusive_parms, args, kwargs, exclusive=True)
...
def _calc_func_signature(func, exclusive_parms, args):
    import swat
    ...
    if exclusive_parms:
        ...
        # skipping status_key
        arg_map = dict(zip(
            filter(lambda x: x != 'status_key', inspect.getargspec(func)[0]), args
        ))
        for parm in exclusive_parms:
            assert parm in arg_map, parm
            func_name += '.' + str(arg_map[parm])
...
def _remove_process_lock(func, exclusive_parms, args, process_thread):
    func_name = _calc_func_signature(func, exclusive_parms, args)
    _process_lock.acquire()
    try:
...
class _ProcessThread(threading.Thread):
    def __init__(self, status_key, func, exclusive_parms, args, kwargs, exclusive):
        self._status_key = status_key
        self._func = func
        self._args = args or []
        self._kwargs = kwargs or {}
        self._exclusive = exclusive
        self._exclusive_parms = exclusive_parms
    ...
    def run(self):
        try:
            ...
            if self._exclusive:
                _remove_process_lock(self._func, self._exclusive_parms, self._args, self)
