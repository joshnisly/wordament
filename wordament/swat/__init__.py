...
def file_response(request, file_path, force_download=False, attachment_override=None):
...
def status_thread(exclusive, exclusive_parms=None):
    def wrap(func):
        @json_request
        def wrap_inner(request, *a, **kw):
            a = (request,) + a
            if exclusive:
                status_key = swat.status.thread_spawn_exclusive(
                    func, exclusive_parms, args=a, kwargs=kw
                )
            else:
...
def _try_generator(generator, exc_handler):
    """ Wraps generator and calls exc_handler(exc_type, exc_value, exc_traceback)
    ...
    while True:
        try:
            ...
            # The generator doesn't have any more values, so just kick out.
            return
        except GeneratorExit:
            # The caller is trying to close the generator. Give the wrapped
            # generator a chance to do its cleanup, but catch its exceptions.
            try:
                generator.close()
            except Exception:
                exc_handler(*sys.exc_info())
            return
        except Exception:
            # Something went wrong! Pass on the information and kick out.
            exc_handler(*sys.exc_info())
            return

def _format_and_log_failed_request(request, tb):
    import sbi_logger
...
def json_request(func):
    ...
    def wrap(request, *a, **kw):
        try:
            ...
            if isinstance(e, ExclusivityError):
                return HttpResponse('Service unavailable', code=503, content_type='text/plain')

            tb = traceback.format_exc()
            if request.json_tracebacks:
                _format_and_log_failed_request(request, tb)
...
def run_standalone(application, host_spec=None, should_reload=True, extra_environ=None,
    ...
    def main(host_spec):
        ...
        server = swat.wsgiserver.CherryPyWSGIServer((host, port), application,
                                                    server_name=host)
        # Put the extra environment on the wsgi server, allowing multiple 
        # servers to run with one application.
        if not extra_environ is None:
            server.environ = dict(extra_environ)
    ...
    # Disable debugging when running under the debugger.
    should_reload = should_reload and not sys.gettrace()
    if should_reload:
...
class Application:
    def __init__(self, urls, show_tracebacks=True, send_500_emails=True,
                 client_facing_error='Something bad happened', json_tracebacks=False):
        ...
        self._json_tracebacks = json_tracebacks
    ...
    def __call__(self, environ, start_response):
        ...
        request.json_tracebacks = self._json_tracebacks
        if request.method == 'POST':
        ...
        if isinstance(response.body, basestring):
            ...
            return _try_generator(
                response.body, lambda *args: self._report_exc(request, *args))
    ...
    def _report_exc(self, request, exc_type, exc_value, exc_traceback):
    ...
    def _get_swat_static_dir(self):
        # Py2exe includes swat_static folder in higher folders than defaulted.
        abs_file = os.path.abspath(__file__)
        if hasattr(sys, 'frozen'):
            parent_dir = os.path.dirname(os.path.dirname(os.path.dirname(abs_file)))
        else:
...
def reverse(func, compiled_urls, args, kwargs):
    for url in compiled_urls:
        ...
        if isinstance(func, basestring):
            target_func = get_wrapped_func(url_func)
...
def _substitute_url_args(compiled_url_spec, args, kwargs):
    ...
    # Do not include ? if we have no query_str
...
class TestTryGenerator(unittest.TestCase):
    class GeneratorError(Exception):
        pass

    def _test(self, generator, expected_results, expected_exceptions,
              max_iterations=None):
        encountered_exceptions = []
        def exc_handler(exc_type, ignored_exc_value, ignored_traceback):
            encountered_exceptions.append(exc_type)

        encountered_results = []
        for item in _try_generator(generator, exc_handler):
            encountered_results.append(item)
        ...
        self.assertEquals(encountered_exceptions, expected_exceptions)
        self.assertEquals(encountered_results, expected_results)
    
    def test_basic(self):
        def simple_generator():
            yield 1
            yield 2
        self._test(simple_generator(), [1, 2], [])
        
        def early_raise():
            if True: #confuse pylint
                raise self.GeneratorError()
            yield 1
        self._test(early_raise(), [], [self.GeneratorError])
        ...
        def middle_raise():
            ...
            if True: #confuse pylint
                raise self.GeneratorError()
            yield 2
        self._test(middle_raise(), [1], [self.GeneratorError])
        ...
        def late_raise():
            try:
                ...
                raise self.GeneratorError()
        self._test(late_raise(), [1], [self.GeneratorError],
                   max_iterations=1)
