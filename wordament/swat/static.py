...
def serve_file(request, path, force_download=False, attachment_override=None):
    ...
    if force_download:
        ...
        if attachment_override:
            attachment_name = attachment_override
        
        content_disposition = 'attachment; filename="%s"' % \
                              attachment_name
        response.headers['Content-Disposition'] = content_disposition
    else:
        assert not attachment_override, attachment_override
...
def _determine_ranges(ranges_str, total_size):
    ...
    for spec in ranges:
        ...
        result_ranges.append({
            'start': first,
            'end': last,
            'spec': 'bytes %i-%i/%i' % (first, last, total_size)
        })
...
class RangesTest(unittest.TestCase):
    ...
    def test_simple(self):
        self._test('bytes=0-499', 1000, 0, 499, 'bytes 0-499/1000')

        self._test('bytes=0-', 1000, 0, 999, 'bytes 0-999/1000')
        self._test('bytes=0', 1000, 0, 0, 'bytes 0-0/1000')
        self._test('bytes=-1', 1000, 999, 999, 'bytes 999-999/1000')
        self._test('bytes=-50', 1000, 950, 999, 'bytes 950-999/1000')

        # Limit to actual file size.
        self._test('bytes=0-499', 400, 0, 399, 'bytes 0-399/400')
